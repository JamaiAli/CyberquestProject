    ...
    if (joined.includes('192.168.1.0/24') || args.includes('-sn') || joined.includes('/24')) {
      ...
      return {
        output: [
          ...
          '',
          '\x1b[35mORACLE > Bien. Tu vois le réseau. Web Server et Mail Server sont ouverts.\x1b[0m',
          '\x1b[35mORACLE > Commence par le Web Server — c\'est le plus facile.\x1b[0m',
        ].join('\n'),
  ...
  if (cmd === 'cd') {
    ...
    return {
      output: [
        ...
        '',
        `\x1b[35mORACLE > Tu es dans ${machine.name}. Commence par : recon\x1b[0m`,
      ].join('\n'),
    ...
    if (args[0] === '/flag.txt' || args[0] === 'flag.txt' || args[0] === '/root/flag.txt') {
      ...
      return {
        output: [
          ...
          '',
          pp.gameWin
            ? '\x1b[35mORACLE > C\'est fini. Les preuves sont entre tes mains.\x1b[0m\n\x1b[35mORACLE > NEXUS Corp ne s\'en remettra pas. Bien joué.\x1b[0m\n\x1b[32m🏆 TU AS COMPLÉTÉ CYBERQUEST ! Tape "exit".\x1b[0m'
            : `\x1b[35mORACLE > ${machine.name} compromise. Continue. Il reste des cibles.\x1b[0m`,
        ].filter(s => s !== '').join('\n'),
