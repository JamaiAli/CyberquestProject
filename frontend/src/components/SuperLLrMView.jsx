  ...
  async function send() {
    ...
    try {
      const res = await fetch('/api/sentinel', {
        ...
        body: JSON.stringify({ messages: history, level: challenge.id }),
      });

      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const result = await res.json();

      setMessages(prev => [...prev, { from: 'llm', text: result.reply }]);
      setLoading(false);

      if (result.solved && !solved) {
        setSolved(true);
        setTimeout(() => {
          setMessages(prev => [...prev, { from: 'flag', text: result.flag }]);
      ...
      setMessages(prev => [...prev, {
        from: 'llm',
        text: "[Erreur] Impossible de joindre SENTINEL. Vérifie que le backend tourne (node server.js) et que la clé GROQ_API_KEY est configurée.",
      }]);
    }
  }
