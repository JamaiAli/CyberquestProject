...
import { processCommand } from './engine/commandEngine.js';
import { getGameState, updateGameState, createInitialState } from './engine/gameState.js';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      setUser(savedUser);
      setSessionId(savedUser);
      const state = getGameState(savedUser);
      setGameState(state);
      if (state.pwnedMachines && state.pwnedMachines.length > 0) {
        setScreen('game');
      } else {
        setScreen('intro');
      }
  ...
  const handleCommand = useCallback(async (command) => {
    ...
    try {
      const result = processCommand(command, gameStateRef.current);
      
      setGameState(prev => {
        const next = result.newState || prev;
        updateGameState(sessionId, next);
      ...
      console.error(err);
      return '\x1b[31m[ERREUR] Erreur lors de l\'exécution de la commande.\x1b[0m';
    }
  }, [sessionId]);

  const handleRestart = () => {
    setGameState(createInitialState(user));
  ...
  const handleLoginSuccess = useCallback(async (token, username) => {
    ...
    const state = getGameState(username);
    setGameState(state);
    setPlayer({ hackerName: 'GHOST', role: 'Hacktiviste', emoji: '🧑‍💻' });
    setScreen(state.pwnedMachines?.length > 0 ? 'game' : 'intro');
  }, []);
